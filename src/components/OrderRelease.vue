<template>
  <div class="order-release">
    <div class="header">
      <div
        class="auction"
        :class="{'pick-up':orderType}"
      >
        <div class='sty'>
          <div
            class="on-sale"
            @click="changeStyle(1);getPartOrder()"
          >竞拍订单</div>
          <div
            class="fin"
            @click="changeStyle(2);getPublishOrder()"
          >发布订单</div>
        </div>
      </div>
      <div
      ...
      >
        <div
          :class="{'day-sty':dayType == 1}"
          @click="changeDay(1)"
        >参拍中</div>
        <div
          :class="{'day-sty':dayType == 2}"
          @click="changeDay(2);getMeetList()"
        >
          待见面
          <div class="msg-pop">{{toMeet.length}}</div>
        </div>
        <div
          :class="{'day-sty':dayType == 3}"
          @click="changeDay(3);getFinOrder()"
        >已结束</div>
      </div>
      <div
        v-if="!orderType"
        ...
        >
          <div class="l1">
            <div class="info">
              <i
                v-if="item.gender == 0"
                class="el-icon-female gender-bg"
              >
                {{convertAge(item.age)}}
              </i>
              <i
          ...
          <div class="l4">
            ...
            <div
              class="check-person sty-color"
              @click="goJoin(item.oid)"
            >
              再次出价
            </div>
          </div>
      ...
      <div v-if="dayType == 3">
        ...
        >
          <div class="l1">
            ...
            <div
              class="l-time evaluate"
              @click="dialogVisible = true; value1 = 5"
            >去评价</div>
            <el-dialog
              title="评价"
